...
def NonMaximalSuppression(img, radius):
    def calculate_max_pixel_value(x, y):
        max_val = -float('inf')
        max_index = (-1, -1)
    ...
    for x in range(offset, img.shape[0] - offset):
        for y in range(offset, img.shape[1] - offset):
            max_value, max_index = calculate_max_pixel_value(x, y)
            img[max_index[0], max_index[1]] = max_value
...
"""
# 1- gradients in both the X and Y directions.
def harris(img, thresh=200, radius=5, verbose=True):
